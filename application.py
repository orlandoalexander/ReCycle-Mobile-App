...
class returnRecyclingInfo(Resource): #class that is a resource - for GET, PUT and DELETE requests
    ...
    def get_category(self):
        for key in self.nouns:
            mycursor.execute("SELECT * FROM Categories WHERE `County` = '%s' AND `Type-main` = '%s' LIMIT 1" % ("Dacorum", key))  # executes query to find the main category type of the user's item. Limit 1 ensures that the results do not flow into the next execution of the db cursor, which would cause an Unread Result Found error
